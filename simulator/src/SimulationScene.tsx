  ...
  useEffect(() => {
    ...
    const rebuild = (w: World) => {
      ...
      for (const [index, target] of w.targets.entries()) {
        ...
        const person = makePerson(outfit[0], { accent: outfit[1], skin: outfit[2] }); person.userData.targetId = target.id; person.userData.originalBodyColor = ((person.children[0] as THREE.Mesh).material as THREE.MeshStandardMaterial).color.clone(); people.set(target.id, person); scene.add(person);
    ...
    return () => { cancelAnimationFrame(frame); observer.disconnect(); renderer.domElement.removeEventListener("pointerdown", select); renderer.domElement.removeEventListener("pointermove", moveDrag); renderer.domElement.removeEventListener("pointerup", endDrag); renderer.domElement.removeEventListener("pointercancel", endDrag); renderer.domElement.removeEventListener("wheel", zoom); renderer.domElement.removeEventListener("dblclick", resetView); window.removeEventListener("keyup", keyUp); window.removeEventListener("blur", onClearKeys); document.removeEventListener("visibilitychange", onClearKeys); for (const visual of sensorVisuals.values()) disposeSensorVisual(visual); sensorVisuals.clear(); radarGeometry.dispose(); radarMaterial.dispose(); floorTexture.dispose(); wallTexture.dispose(); disposeObject(scene); renderer.dispose(); element.replaceChildren(); };
