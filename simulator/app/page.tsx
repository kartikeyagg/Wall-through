...
import type { StereoRig } from "../src/stereo.js";
...
const initial: Settings = { paused: false, selected: "P2", sharing: true, cones: true, links: true, trails: true, vectors: true, patrol: false, autoRotate: false, rotationSpeed: 30, mode: "overview", range: 420, fov: 117, baseline: 0.08, speed: 1 };
...
export default function Home() {
  ...
  const [stats, setStats] = useState({ time: 0, detections: 0, contacts: [] as VisionContact[], heading: 270, sensors: 0, rig: pipeline.current.rig as StereoRig, disparity: 0, sigma: 0 });
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const { detections, tracks, rig } = pipeline.current.update(current, current.time * 1000);
      const contacts = visibleTo(current, config.selected, trackObservations(tracks, vision), config.sharing);
      contactsRef.current = contacts;
      if (now - lastStats > 100) {
        const angle = current.officers.find((officer) => officer.id === config.selected)?.angle ?? 0;
        const mean = (values: number[]) => values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : 0;
        setStats({ time: current.time, detections: tracks.length, contacts, heading: ((angle * 180 / Math.PI) % 360 + 360) % 360, sensors: detections.length, rig, disparity: mean(detections.map((item) => item.disparity)), sigma: mean(tracks.map((item) => item.sigma)) }); lastStats = now;
      }
  ...
  return <main>
    ...
    <div className="workspace"><section className="simulation" aria-label="Simulation workspace">
      ...
      <div className="map-wrap"><SimulationScene world={world} options={sceneOptions} contacts={contactsRef} onSelect={select} onKeyDown={keyDown} onKeyUp={keyUp} onClearKeys={clearKeys} /><div className="map-caption">{settings.mode === "overview" ? "3D GROUND TRUTH · all bodies shown for inspection" : `${settings.selected} STEREO RIG · triangulated tracks and teammate feeds`}</div></div>
      <div className="legend"><span><i className="dot police" />Police stereo rig</span><span><i className="diamond" />Triangulated terrorist</span><span><i className="outline" />Fused shared outline</span><span><i className="trail" />Movement trail</span><span><i className="vector" />Velocity vector</span><span><i className="wall" />Static wall collider</span></div>
      ...
      <section className="panel"><div className="section-heading"><h2>Live vision tracks</h2><span className="tiny">{stats.contacts.length} IN VIEW</span></div><div className="contacts" aria-live="off">{stats.contacts.length ? stats.contacts.map((contact) => <div className={contact.coasting ? "contact coasting" : "contact"} key={contact.targetId}><span className={contact.moving ? "contact-icon moving" : contact.kind === "shared" ? "contact-icon shared" : "contact-icon"}>◇</span><div><strong>{contact.targetId} <span>{contact.kind === "shared" ? "SHARED" : "DIRECT"}</span> <span className="motion">{unitsToMetres(contact.speed).toFixed(1)} m/s · {(((contact.heading * 180 / Math.PI) % 360 + 360) % 360).toFixed(0)}°</span></strong><small>{contact.coasting ? "Predicted — no camera currently resolves this target" : `Live sensor reports: ${contact.observers.join(", ")}`}</small></div><i /></div>) : <p className="empty">No confirmed live tracks in this officer’s view.<br />Rotate, move, or select another officer.</p>}</div></section>
      <section className="panel configuration"><h2>Simulation settings</h2><label>Police members <output>{count}</output><input aria-label="Police members" type="range" min="5" max="10" value={count} onChange={(event) => { const members = Number(event.target.value); setCount(members); reset(members); }} /></label><label>Camera field of view <output>{settings.fov}°</output><input aria-label="Camera field of view" type="range" min="40" max="160" value={settings.fov} onChange={(event) => change("fov", Number(event.target.value))} /></label><label>Stereo baseline <output>{(settings.baseline * 100).toFixed(0)} cm</output><input aria-label="Stereo baseline" type="range" min="2" max="30" step="1" value={Math.round(settings.baseline * 100)} onChange={(event) => change("baseline", Number(event.target.value) / 100)} /></label><label>Camera range <output>{settings.range} units</output><input aria-label="Camera range" type="range" min="100" max="650" step="10" value={settings.range} onChange={(event) => change("range", Number(event.target.value))} /></label><p className="muted">Optics cap detection at {reach.toFixed(0)} units; the range slider can only tighten it.</p><div className="rotation-controls"><button className={settings.autoRotate ? "rotation-toggle scanning" : "rotation-toggle"} aria-pressed={settings.autoRotate} onClick={() => change("autoRotate", !settings.autoRotate)}>{settings.autoRotate ? "Stop automatic rotation" : "Start automatic rotation"}</button><p className="muted">All officers scan in place. Stop preserves headings.</p><label>Rotation speed <output>{settings.rotationSpeed}°/s</output><input aria-label="Rotation speed" type="range" min="5" max="180" step="5" value={settings.rotationSpeed} onChange={(event) => change("rotationSpeed", Number(event.target.value))} /></label></div><div className="toggles">{([ ["cones", "Vision cones"], ["links", "Communication links"], ["trails", "Movement trails"], ["vectors", "Velocity vectors"], ["patrol", "Automatic police patrol"] ] as const).map(([key, label]) => <label key={key}><span>{label}</span><input aria-label={label} type="checkbox" checked={settings[key]} onChange={(event) => change(key, event.target.checked)} /></label>)}</div></section>
    </aside></div>
    <section className="explanation"><div><span className="step">01</span><h3>See</h3><p>A head-mounted stereo pair projects each target into two image planes.</p></div><div><span className="step">02</span><h3>Triangulate</h3><p>Disparity between the two views gives depth, and with it a 3D position and its error.</p></div><div><span className="step">03</span><h3>Track</h3><p>A constant-velocity filter fuses observers into one live track with velocity and heading.</p></div><div><span className="step">04</span><h3>Share</h3><p>Officers see direct contacts and teammate-provided 3D outlines and movement.</p></div></section>
