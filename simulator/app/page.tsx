...
type Settings = SceneOptions & { paused: boolean; sharing: boolean; patrol: boolean; autoRotate: boolean; rotationSpeed: number; speed: number; imu: boolean; cameraFps: number };
const initial: Settings = { paused: false, selected: "P2", sharing: true, cones: true, links: true, trails: true, vectors: true, directionArrows: true, skeletons: true, overlayOpacity: 0.35, patrol: false, autoRotate: false, rotationSpeed: 30, mode: "overview", range: 420, fov: 117, baseline: 0.08, speed: 1, imu: true, cameraFps: 30, sensorRings: true, radarRange: radarReach, hideCleared: false, mouseLook: true };
...
export default function Home() {
  ...
  const reset = useCallback((members = count) => { world.current = createWorld(members); pipeline.current.reset(); localizer.current.reset(); threats.current.reset(); sensorsRef.current = []; localizationsRef.current = []; setLocalization({ estimate: null, error: 0 }); keys.current.clear(); setSettings((current) => ({ ...current, selected: "P2" })); }, [count]);
  ...
  return <main>
    ...
    <div className="workspace"><section className="simulation" aria-label="Simulation workspace">
      ...
      <section className="panel"><div className="section-heading"><h2>Self localization</h2><span className="tiny">{settings.imu ? "STEREO + COMPASS + IMU" : "STEREO + COMPASS"}</span></div><div className="vision-readout"><div><strong>{localized ? unitsToMetres(localized.position.x).toFixed(1) : "—"}<small>m</small></strong><span>LOCAL X</span></div><div><strong>{localized ? unitsToMetres(localized.position.z).toFixed(1) : "—"}<small>m</small></strong><span>LOCAL Z</span></div><div><strong>{localized ? ((localized.orientation.yaw * 180 / Math.PI % 360 + 360) % 360).toFixed(0) : "—"}<small>°</small></strong><span>COMPASS</span></div><div><strong>{(unitsToMetres(localizationError) * 100).toFixed(0)}<small>cm</small></strong><span>SIM ERROR</span></div><div><strong>{localized?.fix ? localized.fix.landmarks : "—"}</strong><span>FEATURES</span></div><div><strong className={localized?.fix && localized.fix.quality < 0.4 ? "warn" : "lime"}>{localized?.fix ? (localized.fix.quality * 100).toFixed(0) : "—"}<small>%</small></strong><span>FIX QUALITY</span></div></div><p className="rig-note">Stereo map features and compass create the primary pose. The optional IMU is then integrated with a deliberately noisy, low-weight correction. FEATURES counts the paintings, panels and floor markings this officer can currently see; the fix tightens as they grow more numerous, more distinctive and more spread out across the view. Turn to face a bare wall and watch the error climb.</p></section>
