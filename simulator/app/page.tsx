...
const initial: Settings = { paused: false, selected: "P2", sharing: true, cones: true, links: true, trails: true, vectors: true, directionArrows: true, skeletons: true, overlayOpacity: 0.35, patrol: false, autoRotate: false, rotationSpeed: 30, mode: "overview", range: 420, fov: 117, baseline: 0.08, speed: 1, imu: true };
...
export default function Home() {
  ...
  const reset = useCallback((members = count) => { world.current = createWorld(members); pipeline.current.reset(); localizer.current.reset(); setLocalization({ estimate: null, error: 0 }); keys.current.clear(); setSettings((current) => ({ ...current, selected: "P2" })); }, [count]);
