...
const initial: Settings = { paused: false, selected: "P2", sharing: true, cones: true, links: true, trails: true, vectors: true, skeletons: true, overlayOpacity: 0.35, patrol: false, autoRotate: false, rotationSpeed: 30, mode: "overview", range: 420, fov: 117, baseline: 0.08, speed: 1 };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const contacts = visibleTo(current, config.selected, trackObservations(tracks, vision, skeletons), config.sharing);
  ...
  return <main>
    ...
    <div className="workspace"><section className="simulation" aria-label="Simulation workspace">
      ...
      <div className="map-wrap"><SimulationScene world={world} options={sceneOptions} contacts={contactsRef} overlays={overlaysRef} onSelect={select} onKeyDown={keyDown} onKeyUp={keyUp} onClearKeys={clearKeys} /><div className="map-caption">{settings.mode === "overview" ? "3D GROUND TRUTH · all bodies shown for inspection" : `${settings.selected} STEREO RIG · triangulated tracks and teammate feeds`}</div></div>
