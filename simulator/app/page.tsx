...
import ControlsGuide from "../src/ControlsGuide";
...
import { createMmWaveRadar } from "../src/mmwave.js";
...
// Read the puck's reach from the radar spec rather than restating it here.
const radarReach = createMmWaveRadar().maxRange;
const guessedParameterCount = guessedParameters().length;
type Settings = SceneOptions & { paused: boolean; sharing: boolean; patrol: boolean; autoRotate: boolean; rotationSpeed: number; speed: number; imu: boolean; gps: boolean; uwb: boolean; environment: "indoor" | "outdoor"; cameraFps: number };
const initial: Settings = { paused: false, selected: "P2", sharing: true, cones: true, links: true, trails: true, vectors: true, directionArrows: true, skeletons: true, overlayOpacity: 0.35, patrol: false, autoRotate: false, rotationSpeed: 30, mode: "overview", range: 420, fov: 117, baseline: 0.08, speed: 1, imu: true, gps: false, uwb: false, environment: "indoor", cameraFps: 30, sensorRings: true, radarRange: radarReach, hideCleared: false, mouseLook: true };
const rigOptions = (settings: Settings) => ({ rig: { baseline: settings.baseline }, fov: settings.fov * Math.PI / 180, range: settings.range, fps: settings.cameraFps });
...
export default function Home() {
  ...
  // The minimap draws officers from their PUBLISHED pose, never their true
  // position, so it inherits the localization error the operator actually has.
  const localizationsRef = useRef<LocalizationEstimate[]>([]);
  ...
  useEffect(() => { pipeline.current.configure({ fps: settings.cameraFps }); }, [settings.cameraFps]);
  useEffect(() => {
    world.current = createWorld(7, { environment: initial.environment });
    ...
    const tick = (now: number) => {
      ...
      const trackReports = trackObservations(tracks, vision, skeletons, sensors);
      threats.current.observeSight(trackReports.filter((report) => !report.coasting && report.observers.length > 0).map((report) => report.targetId), current.time * 1000);
      const observations = threats.current.annotate(trackReports);
  ...
  const cameraRateClass = stats.captureRate < settings.cameraFps * 0.85 ? "warn" : "lime";
  ...
  return <main>
    ...
    <section className="intro"><div><p className="eyebrow">COLLABORATIVE VISION LAB / 3D</p><h1>See beyond your line of sight<span>.</span></h1><p>Head-mounted stereo cameras and thrown mmWave pucks detect and track moving bodies in a shared three-dimensional world — and the officer decides which of them is a threat.</p></div><div className="scenario"><span>SCENARIO {settings.environment === "indoor" ? "001" : "002"}</span><strong>{settings.environment === "indoor" ? "Divided hall — 3D" : "Open training ground — 3D"}</strong><small>{settings.environment === "indoor" ? "Stereo triangulation · Opaque barriers · Motion tracking" : "Open sky · Optional GPS · Motion tracking"}</small></div></section>
    <div className="workspace"><section className="simulation" aria-label="Simulation workspace">
      <div className="map-toolbar"><div className="tabs" aria-label="View mode"><button className={settings.mode === "overview" ? "active" : ""} aria-pressed={settings.mode === "overview"} onClick={() => change("mode", "overview")}>World overview</button><button className={settings.mode === "glasses" ? "active" : ""} aria-pressed={settings.mode === "glasses"} onClick={() => change("mode", "glasses")}>Officer glasses</button></div><ControlsGuide /><span className="map-state"><i className={settings.paused ? "paused" : ""} />{settings.paused ? "PAUSED" : "LIVE"}<time>{stats.time.toFixed(1)}s</time></span></div>
      <div className="map-wrap"><SimulationScene world={world} options={sceneOptions} contacts={contactsRef} awareness={awarenessRef} overlays={overlaysRef} sensors={sensorsRef} onSelect={select} onToggleThreat={toggleThreat} onKeyDown={keyDown} onKeyUp={keyUp} onClearKeys={clearKeys} /><Minimap world={world} localizations={localizationsRef} contacts={awarenessRef} sensors={sensorsRef} options={sceneOptions} onSelect={select} /><div className="map-caption">{settings.mode === "overview" ? "3D GROUND TRUTH · all bodies shown for inspection" : `${settings.selected} STEREO RIG · triangulated tracks and teammate feeds`}</div></div>
      ...
      <section className="panel"><div className="section-heading"><h2>Self localization</h2><span className="tiny">STEREO + COMPASS{settings.imu ? " + IMU" : ""}{localized?.sources.gps ? " + GPS" : ""}</span></div><div className="vision-readout"><div><strong>{localized ? unitsToMetres(localized.position.x).toFixed(1) : "—"}<small>m</small></strong><span>LOCAL X</span></div><div><strong>{localized ? unitsToMetres(localized.position.z).toFixed(1) : "—"}<small>m</small></strong><span>LOCAL Z</span></div><div><strong>{localized ? ((localized.orientation.yaw * 180 / Math.PI % 360 + 360) % 360).toFixed(0) : "—"}<small>°</small></strong><span>COMPASS</span></div><div><strong>{settings.environment === "outdoor" ? unitsToMetres(localizationError).toFixed(1) : (unitsToMetres(localizationError) * 100).toFixed(0)}<small>{settings.environment === "outdoor" ? "m" : "cm"}</small></strong><span>SIM ERROR</span></div><div><strong>{localized?.fix ? localized.fix.landmarks : "—"}</strong><span>FEATURES</span></div><div><strong className={localized?.fix && localized.fix.quality < 0.4 ? "warn" : "lime"}>{localized?.fix ? (localized.fix.quality * 100).toFixed(0) : "—"}<small>%</small></strong><span>VISUAL FIX</span></div>{localized?.gps ? <div><strong>{unitsToMetres(localized.gps.sigma).toFixed(1)}<small>m</small></strong><span>GPS σ</span></div> : null}</div><p className="rig-note">{settings.environment === "outdoor" ? "Open sky has no mapped visual landmarks. Enable GPS in settings to combine a simulated satellite position fix with the weak visual estimate; the compass still supplies heading." : "Stereo map features and compass create the primary pose. The optional IMU is then integrated with a deliberately noisy, low-weight correction. FEATURES counts the paintings, panels and floor markings this officer can currently see; the fix tightens as they grow more numerous, more distinctive and more spread out across the view. Turn to face a bare wall and watch the error climb."}</p>{guessedParameterCount > 0 ? <p className="rig-note">MODEL PARAMETERS: {guessedParameterCount} unmeasured guesses. See the simulation guide for provenance.</p> : null}</section>
      ...
      <div className="radar-readout"><div><strong>{stats.returns}</strong><span>RETURNS/FR</span></div><div><strong className="violet">{stats.radarTracks}</strong><span>RADAR TRACKS</span></div><div><strong>{unitsToMetres(settings.radarRange).toFixed(0)}<small>m</small></strong><span>RADAR REACH</span></div></div>
      ...
      <div className="vision-readout"><div><strong>{(stats.rig.baseline * 100).toFixed(1)}<small>cm</small></strong><span>BASELINE</span></div><div><strong>{stats.rig.focalPx.toFixed(0)}<small>px</small></strong><span>FOCAL</span></div><div><strong>{stats.disparity.toFixed(1)}<small>px</small></strong><span>DISPARITY</span></div><div><strong>{unitsToMetres(reach).toFixed(1)}<small>m</small></strong><span>REACH</span></div><div><strong>{unitsToMetres(stats.sigma).toFixed(2)}<small>m</small></strong><span>DEPTH σ</span></div><div><strong>{stats.rig.imageWidth}<small>×{stats.rig.imageHeight}</small></strong><span>SENSOR</span></div><div><strong className={cameraRateClass}>{stats.captureRate.toFixed(1)}<small>fps</small></strong><span>CAMERA FPS</span></div></div>
      ...
      <p className="muted">The system flags every body it detects. It cannot tell a terrorist from a bystander — that call is yours. A clearance is vouched only while the person stays in sight. Clear a false positive here or by clicking the person in the 3D view; the whole team sees it.</p>
      <div className="contacts" aria-live="off">{stats.contacts.length ? stats.contacts.map((contact) => { const cleared = contact.threat === "cleared"; return <div className={`contact${contact.coasting ? " coasting" : ""}${cleared ? " cleared" : ""}`} key={contact.targetId}><span className={cleared ? "contact-icon cleared" : contact.radar && !contact.stereo ? "contact-icon radar" : contact.skeleton ? "contact-icon posed" : contact.moving ? "contact-icon moving" : contact.kind === "shared" ? "contact-icon shared" : "contact-icon"}>{cleared ? "○" : "◇"}</span><div><strong>{contact.targetId} <span>{cleared ? "CLEARED" : contact.kind === "shared" ? "SHARED" : "DIRECT"}</span> {contact.radar ? <span className="radar-tag">{contact.stereo ? "FUSED" : "RADAR"}</span> : null} {contact.skeleton && !cleared ? <span className="pose">POSE {(contact.skeleton.confidence * 100).toFixed(0)}%</span> : null} <span className="motion">{unitsToMetres(contact.speed).toFixed(1)} m/s · {(((contact.heading * 180 / Math.PI) % 360 + 360) % 360).toFixed(0)}°</span></strong><small>{cleared ? `Not a terrorist — cleared by ${contact.clearedBy ?? "an officer"}` : contact.restoredBy === "system" ? "System re-flagged — clearance lapsed after leaving sight" : contact.coasting ? "Predicted — no sensor currently resolves this target" : `Live sensor reports: ${contact.observers.join(", ")}`}</small></div><button className={cleared ? "detag restore" : "detag"} onClick={() => toggleThreat(contact.targetId)} aria-label={cleared ? `Re-flag ${contact.targetId} as a threat` : `Clear ${contact.targetId} as a false positive`}>{cleared ? "Re-flag" : "Not a terrorist"}</button></div>; }) : <p className="empty">No confirmed live tracks in this officer’s view.<br />Rotate, move, or select another officer.</p>}</div>
      <div className="score-readout"><div><strong className="lime">{stats.score.correctlyCleared}</strong><span>RIGHT CLEARS</span></div><div><strong className="warn">{stats.score.wronglyCleared}</strong><span>WRONG CLEARS</span></div><div><strong>{stats.score.correctlyFlagged}</strong><span>HOSTILE HELD</span></div><div><strong>{stats.score.bystandersLeftFlagged}</strong><span>STILL FLAGGED</span></div></div>
      <p className="overlay-note">Lab scoring only, against ground truth no officer or sensor can read: one of the three people in this scene is hostile. Clearing that one is the dangerous mistake; leaving a bystander flagged is only a nuisance.</p></section>
      <section className="panel configuration"><h2>Simulation settings</h2><label>Environment<select aria-label="Environment" value={settings.environment} onChange={(event) => reset(count, event.target.value as Settings["environment"])}><option value="indoor">Divided hall (indoor)</option><option value="outdoor">Open training ground (outdoor)</option></select></label><p className="muted">Changing environment starts a fresh scene.</p><label>Police members <output>{count}</output><input aria-label="Police members" type="range" min="5" max="10" value={count} onChange={(event) => { const members = Number(event.target.value); setCount(members); reset(members); }} /></label><label>Camera field of view <output>{settings.fov}°</output><input aria-label="Camera field of view" type="range" min="40" max="160" value={settings.fov} onChange={(event) => change("fov", Number(event.target.value))} /></label><label>Stereo baseline <output>{(settings.baseline * 100).toFixed(0)} cm</output><input aria-label="Stereo baseline" type="range" min="2" max="30" step="1" value={Math.round(settings.baseline * 100)} onChange={(event) => change("baseline", Number(event.target.value) / 100)} /></label><label>Camera range <output>{settings.range} units</output><input aria-label="Camera range" type="range" min="100" max="650" step="10" value={settings.range} onChange={(event) => change("range", Number(event.target.value))} /></label><label>Camera frame rate <output>{settings.cameraFps} fps</output><input aria-label="Camera frame rate" type="range" min="1" max="60" step="1" value={settings.cameraFps} onChange={(event) => change("cameraFps", Number(event.target.value))} /></label><p className="muted">A slower camera means tracks coast between captures and pose overlays age.</p><p className="muted">Detection reaches {reach.toFixed(0)} units — the tighter of the {optics.toFixed(0)}-unit optical limit and this slider.</p><div className="rotation-controls"><button className={settings.autoRotate ? "rotation-toggle scanning" : "rotation-toggle"} aria-pressed={settings.autoRotate} onClick={() => change("autoRotate", !settings.autoRotate)}>{settings.autoRotate ? "Stop automatic rotation" : "Start automatic rotation"}</button><p className="muted">All officers scan in place. Stop preserves headings.</p><label>Rotation speed <output>{settings.rotationSpeed}°/s</output><input aria-label="Rotation speed" type="range" min="5" max="180" step="5" value={settings.rotationSpeed} onChange={(event) => change("rotationSpeed", Number(event.target.value))} /></label></div><div className="toggles">{([ ["cones", "Vision cones"], ["links", "Communication links"], ["trails", "Movement trails"], ["vectors", "Velocity vectors"], ["directionArrows", "Direction arrows"], ["skeletons", "Skeleton overlay"], ["sensorRings", "Radar coverage rings"], ["hideCleared", "Hide cleared people"], ["mouseLook", "Mouse look (glasses)"], ["imu", "IMU integration"], ["patrol", "Automatic police patrol"] ] as const).map(([key, label]) => <label key={key}><span>{label}</span><input aria-label={label} type="checkbox" checked={settings[key]} onChange={(event) => change(key, event.target.checked)} /></label>)}<label><span>GPS position fix (outdoor)</span><input aria-label="GPS position fix" type="checkbox" checked={settings.gps} disabled={settings.environment !== "outdoor"} onChange={(event) => change("gps", event.target.checked)} /></label><label><span>UWB ranging (future)</span><input aria-label="UWB ranging (future)" type="checkbox" checked={settings.uwb} disabled readOnly /></label></div><p className="muted">GPS starts off and is available only under open sky. UWB is reserved as an additional feature and is off.</p></section>
    </aside></div>
