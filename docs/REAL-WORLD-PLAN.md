...
5. **Radar realism.** A real 60 GHz chip covers 120°, not 240°. TI's people-tracking reference configuration reaches 6 m, not 18 m. An office plasterboard wall costs 11.8–31.6 dB one way at 60 GHz in NIST's measurement, and an echo crosses it twice. The simulator charges 7.5 dB once. Returns come as a noisy point cloud with ghost points, not one clean point per person.
...
| 12 | **Radar field of view and range** | One puck covers 240° out to 18 m (`mmwave.js` defaults). | The TI IWR6843AOP gives 120° azimuth × 120° elevation. TI's people-tracking reference design tracks up to 20 people with a 6 m default range, which chirp configuration can extend ([TI](https://www.ti.com/tool/TIDEP-01000), [TI EVM](https://www.ti.com/tool/IWR6843AOPEVM)). | A single-chip puck sees a third of what the sim shows. 360° coverage needs three boards. |
| 13 | **Radar through walls** | `returnStrengthDb` subtracts 7.5 dB **once** per wall. | At 60.5 GHz, NIST measured an office plasterboard wall (two 12.7 mm sheets, an air gap and metal studs) at **11.8–31.6 dB one way**, rising with incidence angle. Theory gives about 5.5 dB at normal incidence ([NIST](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=929131)). A radar echo crosses the wall twice, so the round trip costs at least 11 dB per wall, and far more at oblique angles. | Through-wall reach is overstated, especially through two walls or at a slant. |
...
```text
 ...
 ┌───────────────────────────────────────────┐            ┌──────────────────────────────┐
 │ Helmet: stereo cam + IMU (OAK-D Pro W)    │            │ 60 GHz radar ×1–3 (IWR6843AOP)│
...
- **Radar.** Start with a TI **IWR6843AOPEVM** (60–64 GHz, 120° × 120° FOV, point cloud and tracks over USB/UART; [TI](https://www.ti.com/tool/IWR6843AOPEVM)). Run TI's 3D people-tracking firmware first, then tune the chirp for range. Covering 360° takes three boards at 120° each. The **IWRL6432** (57–64 GHz, low-power modes) is the battery-life upgrade ([TI](https://www.ti.com/product/IWRL6432)).
...
- **Mechanics.** A self-righting shell so the radar ends up facing horizontally. The radome must be transparent at 60 GHz, which constrains its material and thickness *(est.; test it)*.
...
| `mmwave.js` | TI people-tracking firmware on the puck. Keep the existing EKF, gating and radar-to-stereo association on the team side; they are already written for real measurements. | Own code |
...
| **R5** Radar puck | Through-wall tracks | IWR6843AOP EVM, controller, marker, UWB; drywall, brick and metal-stud walls | Measured range through 0, 1 and 2 walls, ghost rate, stationary dropout time, and localization error. The simulator's radar model is re-fit to them. |
...
| A9 | **Radar realism.** 120° FOV per board, with 1–3 boards per puck. 6 m default range. **Two-way**, angle-dependent wall loss (2 × one-way loss, rising from about 5.5 dB one way at normal incidence toward the 11.8–31.6 dB NIST measured). A point cloud of several points per body. Multipath ghosts mirrored across nearby walls. Puck yaw from the IMU with an error term, instead of the truth. | `mmwave.js`, `vision.js` (`radarFrame`) | TI and NIST figures in §2, until R5 | Reach through one and two walls matches the budget. The tracker holds one track per body with ghosts present. |
...
- **Radio licensing.** 57–64 GHz radar and UWB are licensed differently in each country. Check with the national regulator (for example, WPC in India or the FCC in the US) before any field use.
...
| Radar at 60 GHz does not get through the walls you care about (brick, concrete, metal studs) | Measure in R5 first. Consider lower-frequency through-wall radars for thick walls. |
...
- TI people counting and tracking design: https://www.ti.com/tool/TIDEP-01000
- TI IWR6843AOPEVM: https://www.ti.com/tool/IWR6843AOPEVM
- TI IWRL6432: https://www.ti.com/product/IWRL6432
- 60 GHz penetration loss (NIST): https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=929131
