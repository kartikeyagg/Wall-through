...
The simulator already gets the stereo optics right, but its default rig — 8 cm baseline, 1280×720 and 117° HFOV, with 0.15 px matching noise — was tuned against a wide-FOV camera. The real head camera is now the Intel RealSense D415: 5.5 cm baseline, 1280×720 depth (1920×1080 RGB), 69.4° × 42.5° HFOV/VFOV, rolling shutter, no built-in IMU ([Intel](https://www.intel.com/content/www/us/en/products/sku/128256/intel-realsense-depth-camera-d415/specifications.html)). Its baseline is 31% smaller than the sim default and its FOV under half as wide, so the sim's rig parameters need retuning, not just its error numbers, once R1 measures the real camera. Intel specifies under 2% depth error at 2 m ([Intel measurement methodology](https://www.mouser.com/pdfdocs/RealSense_DepthQualityTesting.pdf)), inside a 0.5–3 m ideal range. That is a much shorter working range than a wide-baseline camera, so the sim's assumption of good depth well past 3 m needs R1 to confirm it still holds for this camera.
...
| 17 | **Lighting** | Every room is lit. | Tactical entries often happen in the dark. The D415 has an IR laser projector for stereo texture, but no IR flood light for illumination, unlike wide-FOV cameras built for this use case. Active stereo works at short range on texture alone; VIO in the dark struggles further. | Detection range and localization both drop in the dark, likely more than a camera with an IR flood light. |
...
| 19 | **Officer locomotion** | The selected officer's movement snaps straight to a fixed 130 units/s in any input direction and stops instantly on key release (`stepWorld` in `simulation.js`), with no ramp-up or ramp-down. AI-controlled targets already ramp `motion.speed` toward a desired speed using `TARGET_ACCELERATION`/`TARGET_BRAKING` (`simulation.js` lines 18–19, 609), but the officer avatar does not. | Real officers accelerate and decelerate over a human gait cycle, cannot instantly reach full speed sideways (strafe), and plant their feet before changing direction; floor friction, not an instant velocity change, is what stops them. | The officer's motion looks like frictionless ice-skating rather than walking. That understates how long it takes to close on or back away from a doorway, and gives the VIO/AR-swim models (items 1, 16) an unrealistic motion profile to react to. |
| 20 | **Stance and crouch height** | The camera's world-frame height is a fixed rig constant, `rig.mountHeight` (1.7 m by default), read once in `officerPose` (`vision.js` line 48) and never varied by the officer's own state while stepping. | Officers crouch-walk under windows, kneel to clear under furniture, and drop into a low ready stance in CQB — moving the head 30–60 cm lower without necessarily stopping. | A fixed head height gets sightlines over low obstacles and under high ones wrong, and the sim can never show what changes when an officer intentionally lowers their own silhouette and sensor viewpoint. |
...
```text
 ...
 ┌───────────────────────────────────────────┐            ┌──────────────────────────────┐
 │ Helmet: stereo cam + ext. IMU (D415/BNO085)│           │ 24 GHz radar ×1–3 (BGT24MTR12)│
 │         see-through display (AR glasses)  │            │ MCU + Wi-Fi/mesh (ESP32-S3)   │
 │         optional GPS for open sky         │            │ IR/AprilTag marker            │
...
| --- | --- | --- | --- | --- |
| Head stereo camera | **Intel RealSense D415** | Rolling shutter, 1280×720 depth / 1920×1080 RGB, 69.4° × 42.5° HFOV/VFOV, 5.5 cm baseline, 0.5–3 m ideal depth range, IR laser projector for texture (no flood light), no built-in IMU. Depth computed on-camera by the D4 vision ASIC ([Intel](https://www.intel.com/content/www/us/en/products/sku/128256/intel-realsense-depth-camera-d415/specifications.html)). RealSense spun out of Intel in July 2025 and still sells them ([Intel Capital](https://www.intelcapital.com/realsense-completes-spin-out-from-intel-raises-50-million-to-accelerate-ai-powered-vision-for-robotics-and-biometrics/)). | Narrower FOV and shorter ideal range than a wide-baseline camera, so it needs its own error-model measurement in R1 rather than reusing the sim's current default rig. Depth is still computed on the camera. No IMU means one must be added and extrinsically calibrated against the camera. | **RealSense D455/D435i**: built-in IMU, wider FOV, longer range — closer to the sim's original assumptions, at the cost of a larger housing. **Luxonis OAK-D Pro W**: 127° HFOV, global shutter, built-in 9-axis IMU and IR flood light, all in one module ([Luxonis](https://docs.luxonis.com/hardware/products/OAK-D%20Pro%20W)). |
| External IMU (paired with D415) | **Adafruit BNO085 9-DoF breakout** | CEVA/Hillcrest BNO085, the same IMU family as the OAK-D Pro's built-in BNO086, over I2C/UART/SPI. | The D415 has no IMU, so VIO needs one rigidly mounted to the camera housing and calibrated with Kalibr. | A Bosch BMI088/BMI055-class 6-axis IMU, matching what RealSense's own D435i carries, if 9-axis fusion is not needed. |
...
| Outdoor positioning | **Optional GNSS/GPS receiver; off by default** | Requires a clear view of the sky; measure position error in the intended setting. | Provides an outdoor position fix when mapped visual landmarks are sparse. Indoor work continues with visual/inertial localization and shared anchors. | UWB ranging is a future add-on, off by default, after a separate evaluation. |
...
| --- | --- | --- |
| `stereo.js` (optics) | Camera driver and calibration | `librealsense`/`realsense-ros` driver. The factory calibration covers the stereo pair; the external IMU's extrinsics against the camera need Kalibr or OpenCV, since the D415 has no built-in IMU to calibrate against. Keep `stereo.js` as the error model and fit its parameters to measurements. |
...
| `sensors.js` SelfLocalization | **Stereo-inertial VIO:** Isaac ROS Visual SLAM (cuVSLAM) ([NVIDIA](https://github.com/NVIDIA-ISAAC-ROS/isaac_ros_visual_slam)). Alternatively OpenVINS or Basalt, both camera-agnostic given rectified images and IMU data. **Shared frame:** an AprilTag board at the entry point defines `world`. Multi-agent map merging (COVINS-G, Swarm-SLAM; [ref](https://arxiv.org/pdf/2301.07147)) and optional outdoor GPS can constrain position under open sky. UWB ranging is a later add-on, disabled by default. The compass is only a gated prior. | ROS REP-105 frames: `world` → `map` → `odom` → `base_link` |
...
| **R1** One rig on the bench | 3D skeletons from real pixels | RealSense D415, external IMU and Jetson; pose model; stereo lift; log to MCAP; measure GPU headroom with VIO running at the same time | Centroid error against tape-measured truth at 0.5–3 m within 30% of the simulator's σ curve, retuned for the D415's baseline and FOV. Latency measured. |
| **R2** Self-localization | Know where the head is | cuVSLAM stereo-inertial on a helmet; walk closed loops of 50–200 m; AprilTag entry anchor; dark and low-texture tests; optional GPS tests outdoors | Drift reported as % of distance and °/min. Re-anchoring works. The numbers go into the simulator (§3.8). |
...
| **R5** Radar puck | Through-wall tracks | BGT24MTR12 Position2Go EVK, controller, marker; drywall, brick and metal-stud walls | Measured range through 0, 1 and 2 walls, ghost rate, stationary dropout time, and localization error. The simulator's radar model is re-fit to them. |
...
| A13 | **Officer locomotion.** Give the selected officer the same acceleration/braking ramp `targetMotion` already gives AI targets, instead of snapping to full speed. Cap sideways (strafe) speed below forward speed, and require a minimum turn before the officer can move off their current heading at full speed. | `simulation.js` (`stepWorld`) | Human gait acceleration *(est.; reuse `TARGET_ACCELERATION`/`TARGET_BRAKING` as a starting point)*, until measured | Releasing all movement keys decelerates to a stop over a measurable interval instead of instantly. Full-speed strafing is not possible. |
| A14 | **Crouch and stance.** A stance state (standing/crouched/prone) per officer, each with its own head height offset from `rig.mountHeight`, its own move-speed cap, and a transition time between stances instead of an instant snap. `officerPose` reads the current stance height instead of the rig constant. | `simulation.js`, `vision.js` (`officerPose`) | Standing 1.7 m, crouched ~1.1 m, prone ~0.4 m *(est.; measure against a real officer in R1)* | Crouching lowers the camera and detection height within the transition time, and changes what the officer's own sensors can see over or under. |
...
- **Radio licensing.** Check with the national regulator (for example, WPC in India or the FCC in the US) before any field use, and confirm the exact permitted bandwidth around 24 GHz for radar rather than general ISM use *(unverified; confirm per country)*. If UWB is added later, assess its separate licensing then.
...
| Risk | Mitigation |
| --- | --- |
| Indoor shared-frame localization is not good enough to draw people behind walls | Entry anchors and map merging. Evaluate UWB later as an optional constraint. Show uncertainty honestly. Fall back to the 2D minimap when the AR registration budget is exceeded. |
...
- Intel RealSense D415 specifications: https://www.intel.com/content/www/us/en/products/sku/128256/intel-realsense-depth-camera-d415/specifications.html
- Intel RealSense depth-quality testing methodology (accuracy figures): https://www.mouser.com/pdfdocs/RealSense_DepthQualityTesting.pdf
- Luxonis OAK-D Pro W specifications (kept as the alternative camera): https://docs.luxonis.com/hardware/products/OAK-D%20Pro%20W
